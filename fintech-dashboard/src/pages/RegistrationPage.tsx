...
import { DollarSign, Eye, EyeOff, ArrowLeft, User, Calendar, Target, TrendingUp } from 'lucide-react';
...
export default function RegisterPage({ setCurrentPage, setIsAuthenticated }: RegisterPageProps) {
  ...
  const isStep1Valid = formData.name && formData.age && formData.email && formData.password && formData.confirmPassword && formData.password === formData.confirmPassword;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-secondary-50 via-white to-primary-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        {/* Back Button */}
        <button
          onClick={() => currentStep === 1 ? setCurrentPage('landing') : setCurrentStep(1)}
          className="flex items-center space-x-2 text-gray-600 hover:text-gray-900 mb-8 transition-colors"
        >
          <ArrowLeft className="w-4 h-4" />
          <span className="text-sm">{currentStep === 1 ? 'Back to home' : 'Previous step'}</span>
        </button>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-gray-500">{currentStep === 1 ? 'Account Details' : 'Financial Profile'}</span>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-card border border-gray-200 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {currentStep === 1 ? (
              ...
              <>
                <div>
                  <label htmlFor="financialGoals" className="block text-sm font-medium text-gray-700 mb-2">
                    <div className="flex items-center space-x-2">
